...
export default function ViewAllCarPopUpTable(props) {
    ...
    const loadCarDetails = async (carId, brand, numOfp, TransType, fuelType, regNum, color, priceDaily, priceMonthly, freeMileage, pOfExtraKm) => {
        ...
        let res1 = await VehicleService.getCarImage(carId, "Front");
        if (res1.status === 200) {
            frontImage = URL.createObjectURL(res1.data)
        }
        let res2 = await VehicleService.getCarImage(carId, "Back");
        if (res1.status === 200) {
            backImage = URL.createObjectURL(res2.data)
        }
        let res3 = await VehicleService.getCarImage(carId, "Side");
        if (res1.status === 200) {
            sideImage = URL.createObjectURL(res3.data)
        }
        props.data.changeStateCarDetails(carId, brand, numOfp, TransType, fuelType, regNum, color, priceDaily, priceMonthly, freeMileage, pOfExtraKm, frontImage, backImage, sideImage);
    ...
    const getAllCars = async () => {
        ...
        let res = await VehicleService.getAllCar();
